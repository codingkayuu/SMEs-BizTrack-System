    ...
    const handleExport = () => {
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                1: { cellWidth: 80, halign: 'right', textColor: [124, 58, 237] }
            }
        });
        ...
        autoTable(doc, {
            ...
            foot: [[
                { content: 'TOTAL', colSpan: 4, styles: { halign: 'right', fontStyle: 'bold', fillColor: [124, 58, 237], textColor: [255, 255, 255] } },
                { content: formatCurrency(filteredExpenses.reduce((sum, e) => sum + e.amount, 0)), styles: { halign: 'right', fontStyle: 'bold', fillColor: [124, 58, 237], textColor: [255, 255, 255] } }
            ]],
            footStyles: {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 stagger-1 animate-fade-in-up">
                ...
                <Card hoverEffect className="p-6">
                    ...
                    <div className="mt-4 flex items-center">
                        <span className="text-xs font-semibold bg-purple-50 text-purple-700 px-2.5 py-0.5 rounded-full border border-purple-100">+5% vs last month</span>
                    </div>
                </Card>
                ...
                {loading ? (
                    ...
                    <div className="p-16 text-center">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-purple-50 mb-4 animate-pulse">
                            <TrendingDown className="h-8 w-8 text-[#7C3AED]" />
                        </div>
                        ...
                        <table className="hidden md:table min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-100 dark:divide-gray-800">
                                {filteredExpenses.map((expense, index) => (
                                    <tr key={expense.id} style={{ animationDelay: `${index * 50}ms` }} className="hover:bg-purple-50/50 dark:hover:bg-purple-900/10 transition-colors animate-fade-in-up">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-medium">{formatDate(expense.date)}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700 border border-purple-100 dark:border-purple-900/20">
                                                {expense.category}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-[#7C3AED] text-right">-{formatCurrency(expense.amount)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center justify-end space-x-3">
                                                <button onClick={() => openModal(expense)} className="text-gray-400 hover:text-[#7C3AED] transition-colors p-1 hover:bg-purple-50 rounded-full"><Edit className="h-4 w-4" /></button>
                                                <button onClick={() => handleDelete(expense.id)} className="text-gray-400 hover:text-[#7C3AED] transition-colors p-1 hover:bg-purple-50 rounded-full"><Trash2 className="h-4 w-4" /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            ...
                                            <h4 className="font-semibold text-gray-900 dark:text-gray-100 mt-1">{expense.description || 'Unspecified Expense'}</h4>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700 border border-purple-100 dark:border-purple-900/20">
                                            {expense.category}
                                        </span>
                                        <span className="text-xs text-gray-400 capitalize">• {expense.payment_method}</span>
                                    </div>
